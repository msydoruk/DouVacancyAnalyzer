...
public class VacancyAnalysisService : IVacancyAnalysisService
{
    ...
    public async Task<VacancyAnalysisResult> AnalyzeVacancyAsync(Vacancy vacancy, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            await Task.Delay(AnalysisConstants.AnalysisRetryDelayMs, cancellationToken);
    ...
    private async Task<CategoryAnalysisResult?> PerformCategoryAnalysisAsync(Vacancy vacancy, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var jsonStart = responseText.IndexOf(AnalysisConstants.JsonStartChar);
            var jsonEnd = responseText.LastIndexOf(AnalysisConstants.JsonEndChar);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        for (int index = 0; index < vacancies.Count; index++)
        {
            ...
            var truncatedTitle = vacancy.Title.Length > AnalysisConstants.VacancyTitleTruncateLength
                ? vacancy.Title.Substring(0, AnalysisConstants.VacancyTitleTruncateShortLength) + AnalysisConstants.TruncateSuffix
                : vacancy.Title;
            ...
            try
            {
                ...
                processedCount++;
                var progressPercent = AnalysisConstants.AnalysisProgressBasePercent +
                    (int)((processedCount / (double)totalVacancies) * AnalysisConstants.AnalysisProgressRange);
    ...
    private bool IsVacancyMatch(VacancyAnalysisResult analysis)
    {
        ...
        if (analysis.IsModernStack ?? false)
        {
            bonusScore += AnalysisConstants.ModernStackBonusScore;
            reasons.Add($"✅ Modern stack (+{AnalysisConstants.ModernStackBonusScore})");
        }
        else
        {
        ...
        {
            bonusScore += AnalysisConstants.AcceptableEnglishBonusScore;
            reasons.Add($"✅ Acceptable English (+{AnalysisConstants.AcceptableEnglishBonusScore})");
        }
        else
        {
        ...
        {
            bonusScore += AnalysisConstants.NoTimeTrackerBonusScore;
            reasons.Add($"✅ No time tracker (+{AnalysisConstants.NoTimeTrackerBonusScore})");
        }
        else
        {
        ...
        _logger.LogInformation("Match result: TRUE. Bonus score: {Score}/{MaxScore}. Details: {Reasons}",
            totalScore, AnalysisConstants.MaxBonusScore, string.Join(", ", reasons));
